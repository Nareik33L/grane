...
socket is destroyed when it elapses). MySQL `max_execution_time` is set later
as a backup so the server cannot return a successful interrupted SLEEP/BENCHMARK
