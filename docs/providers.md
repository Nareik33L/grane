...
| `non_additive_dimension` (`window_agg` / `window_choice` `max` or `min`; `group_by` / `window_groupings` entities) | `additive: semi` with an explicit `semi_additive` block: `window`, `group_by` (foreign / continuing-series entities; **empty keeps one snapshot for the whole result**, MetricFlow's default) and the dimension's declared `time_granularity`. A `group_by` of the model's own primary or unique entity is skipped: that key is one snapshot row. Filters and the time range apply before the snapshot is chosen. |
